...
(function() {
  ...
  function makeBtn(seconds, svg, label, side) {
    ...
    btn.onmouseenter = function() { btn.style.background = 'color-mix(in srgb, var(--red) 85%, transparent)'; btn.style.borderColor = 'color-mix(in srgb, var(--red) 40%, transparent)'; };
...
export default function TVPage({
  ...
  useEffect(() => {
    let mounted = true;
    tmdbFetch(`/tv/${item.id}/videos`, apiKey, { noLang: true })
      .then((data) => {
        if (!mounted) return;
  ...
  // Preconnect to saved source domain on mount
  useEffect(() => {
    const saved = storage.get("playerSource");
    const src = PLAYER_SOURCES.find((s) => s.id === saved);
    if (!src) return;
    try {
      const domain = new URL(src.movieUrl(1)).origin;
    ...
    window.electron
      ...
      .then((res) => {
        ...
        if (res?.ok && res.url) {
          ...
          setResolveError(res?.error || t("tv.sourceErrorDesc"));
        }
      })
      .catch((e) => {
  ...
  // Auto-failover on AllManga resolve error
  useEffect(() => {
    if (!resolveError || !isAsync) return;
    triedSourcesRef.current.add(playerSource);
    failoverCountRef.current += 1;
    if (failoverCountRef.current >= SOURCE_FALLBACK_ORDER.length) return;
    const next = getNextSource(playerSource);
    if (next && next.id !== playerSource) {
      setPlayerSource(next.id);
      storage.set("playerSource", next.id);
    }
  }, [resolveError]);
  ...
  // Attach webview/iframe load events + auto-failover on error.
  // Also poll for video duration so AniSkip markers appear without waiting for the 5s progress tick.
  useEffect(() => {
    if (!playing) return;
    const wv = webviewRef.current;
    if (!wv) return;
    const isWebview = typeof wv.addEventListener === "function" && "did-finish-load" in wv;
    const loadEvent = isWebview ? "did-finish-load" : "load";
    const failEvent = isWebview ? "did-fail-load" : null;
    const onSuccess = () => {
      setWebviewLoading(false);
      triedSourcesRef.current.clear();
      failoverCountRef.current = 0;
      storage.set("playerSource", playerSource);
    };
    const onFail = isWebview
      ? (e) => {
          setWebviewLoading(false);
      ...
      : null;
    wv.addEventListener(loadEvent, onSuccess);
    if (isWebview && failEvent) wv.addEventListener(failEvent, onFail);
    ...
    return () => {
      wv.removeEventListener(loadEvent, onSuccess);
      if (isWebview && failEvent) wv.removeEventListener(failEvent, onFail);
      clearInterval(pollDuration);
    };
  ...
  useEffect(() => {
    const wv = webviewRef.current;
    const isWebview = wv && typeof wv.addEventListener === "function" && "did-finish-load" in wv;
    if (!wv || !playing || playerSource !== "allmanga" || !isWebview) return;
          ...
          <div className="detail-hero">
            ...
            <div className="detail-content">
              ...
              <div className="detail-info">
                <div className="detail-type">{t("tv.title")}</div>
                ...
                {d.tagline && <div className="detail-tagline">"{d.tagline}"</div>}
                {rating.cert && (
                  <div
                ...
                <div className="detail-overview-wrap">
                  <p className="detail-overview">
                    {displayOverview && displayOverview.length > 200
                      ? displayOverview.slice(0, 200) + "…"
                      : displayOverview}
                  </p>
                  {displayOverview && displayOverview.length > 200 && (
                    <button
                      className="overview-more-btn"
                      onClick={() =>
                        document
                          .getElementById("synopsis-section")
                  ...
                  {trailerKey &&
                    (restricted ? (
                      <button
                        ...
                        disabled
                        title={t("tv.inappropriate")}
                      >
<LockIcon size={14} /> {t("tv.trailer")}
                      </button>
                    ) : (
                      <button
                      ...
                      >
                        <TrailerIcon /> {t("tv.trailer")}
                      </button>
                    ))}
                  <button className="btn btn-secondary" onClick={onSave}>
                    {isSaved ? <BookmarkFillIcon /> : <BookmarkIcon />}
                    {isSaved ? t("tv.saved") : t("tv.save")}
                  </button>
            ...
            <div className="section">
              ...
              >
                <span className="tag tag-red">
                  {t("tv.seasonEp", { season: selectedSeason, episode: selectedEp.episode_number })}
                </span>
                ...
                {currentEpWatched ? (
                  ...
                  >
                    <WatchedIcon size={14} /> {t("tv.watched")}
                  </button>
                ) : (
                  <button
                ...
                {webviewLoading && !resolveError && (
                  ...
                  >
                    ...
                    <span style={{ fontSize: 14, color: "var(--text2)" }}>
                      {resolvingUrl
                        ? t("tv.lookingUp")
                        : t("tv.loadingSource", { source: PLAYER_SOURCES.find((s) => s.id === playerSource)?.label ?? "source" })}
                    </span>
                  </div>
                )}
...
<WarningIcon size={28} />
                    <span style={{ fontSize: 14, color: "var(--text2)" }}>
                      {t("tv.sourceErrorDesc")}
                    </span>
                    <span style={{ fontSize: 12, color: "var(--text3)" }}>
                      {resolveError}
                    </span>
                    <span style={{ fontSize: 12, color: "var(--text3)" }}>
                      {t("tv.tryDifferentSource")}
                    </span>
                  </div>
                )}
                ...
                <div className="player-overlay-group">
                  ...
                  {playerSource === "allmanga" && (
                    <button
                      ...
                      title={t("tv.toggleSubDub")}
                    >
                      {dubMode === "sub" ? t("tv.sub") : t("tv.dub")}
                    </button>
                  )}
                  ...
                  <button
                    ...
                    title={pipOpen ? t("tv.closePopOut") : t("tv.popOut")}
                    disabled={
                      !pipOpen &&
                      (webviewLoading || !!(isAsync && !resolvedPlayerUrl))
                    }
                ...
                >
                  ...
                  {!supportsProgress && (
                    <span
                      ...
                      title={t("tv.noTracking")}
                    >
                      {t("tv.noTrackingLabel")}
                    </span>
                  )}
                </button>
                ...
                {/* AniSkip manual prompt, rendered in sinex UI, outside webview */}
                {skipPrompt && (
                  <button
                    ...
                    onMouseEnter={(e) => {
                      e.currentTarget.style.background = "color-mix(in srgb, var(--red) 85%, transparent)";
                      e.currentTarget.style.borderColor = "color-mix(in srgb, var(--red) 50%, transparent)";
                    }}
                    onMouseLeave={(e) => {
                  ...
                  >
                    ...
                    >
                      {skipPrompt === "intro" ? t("tv.intro") : t("tv.outro")}
                    </span>
                  </button>
                )}
              </div>
                ...
                (() => {
                  ...
                  return epPct > 0 || hasMarkers ? (
                    <div className="progress-bar-row">
                      ...
                      <span style={{ fontSize: 12, color: "var(--text3)" }}>
                        {epPct > 0 ? t("tv.watchedPercent", { pct: epPct.toFixed(0) }) : ""}
                      </span>
                    </div>
                  ) : null;
                })()}
                ...
                {seasons.map((s) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {s.season_number === 0
                        ? t("tv.specials")
                        : t("tv.seasonLabel", { number: s.season_number })}
                    </button>
                  );
                })}
              </div>
...
<WarningIcon size={16} />
      ...
      {d && (
        <div className="section details-section">
          <div className="section-title">{t("tv.details")}</div>
          <div className="details-grid">
            {d.status && (
              <div className="detail-item">
                <span className="detail-item-label">{t("tv.status")}</span>
                <span className="detail-item-value">{d.status}</span>
              </div>
            )}
            {d.original_name && (
              <div className="detail-item">
                <span className="detail-item-label">{t("tv.originalTitle")}</span>
                <span className="detail-item-value">{d.original_name}</span>
              </div>
            )}
            {d.original_language && (
              <div className="detail-item">
                <span className="detail-item-label">{t("tv.originalLanguage")}</span>
                <span className="detail-item-value">{d.original_language.toUpperCase()}</span>
              </div>
            )}
            {d.origin_country?.length > 0 && (
              <div className="detail-item">
                <span className="detail-item-label">{t("tv.country")}</span>
                <span className="detail-item-value">{d.origin_country.join(", ")}</span>
              </div>
            )}
            {d.spoken_languages?.length > 0 && (
              <div className="detail-item">
                <span className="detail-item-label">{t("tv.languages")}</span>
                <span className="detail-item-value">{d.spoken_languages.map(l => l.name).join(", ")}</span>
              </div>
            )}
            {d.episode_run_time?.[0] > 0 && (
              <div className="detail-item">
                <span className="detail-item-label">{t("tv.episodeRuntime")}</span>
                <span className="detail-item-value">{d.episode_run_time[0]} min</span>
              </div>
            )}
            {d.number_of_seasons > 0 && (
              <div className="detail-item">
                <span className="detail-item-label">{t("tv.seasons")}</span>
                <span className="detail-item-value">{d.number_of_seasons}</span>
              </div>
            )}
            {d.number_of_episodes > 0 && (
              <div className="detail-item">
                <span className="detail-item-label">{t("tv.episodes")}</span>
                <span className="detail-item-value">{d.number_of_episodes}</span>
              </div>
            )}
            {d.genres?.length > 0 && (
              <div className="detail-item">
                <span className="detail-item-label">{t("tv.genres")}</span>
                <span className="detail-item-value">{d.genres.map(g => g.name).join(", ")}</span>
              </div>
            )}
            {d.networks?.length > 0 && (
              <div className="detail-item">
                <span className="detail-item-label">{t("tv.networks")}</span>
                <span className="detail-item-value">{d.networks.map(n => n.name).join(", ")}</span>
              </div>
            )}
          </div>
        </div>
      ...
      {/* ── Synopsis ── */}
      {displayOverview && (
        <div className="synopsis-section" id="synopsis-section">
          <div className="section-title">{t("tv.synopsis")}</div>
          <p className="synopsis-text">{displayOverview}</p>
        </div>
      )}
      ...
      {epMenu && (
        <ContextMenu
          ...
          watchedLabel={t("tv.markAsWatched")}
          unwatchedLabel={t("tv.markAsUnwatched")}
      ...
      <div className="episode-thumb">
        ...
        {restricted ? (
          <div className="episode-restricted-overlay">
            <LockIcon size={18} /><span>{t("tv.inappropriate")}</span>
          </div>
        ) : epUnreleased ? (
          <div className="episode-restricted-overlay">
            <LockIcon size={18} /><span>{t("tv.unreleased")}</span>
          </div>
        ) : isPlaying ? (
