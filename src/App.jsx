...
import { tmdbFetch, setApiErrorHandlers, DEFAULT_API_KEY } from "./utils/api";
import { applyTheme } from "./utils/appearance";
import { collectBackupData } from "./utils/backup";
import { CelebrationIcon, BackIcon, SearchIcon, HomeIcon, HistoryIcon, DownloadsQueueIcon, SettingsIcon, SineXLogo } from "./components/Icons";
import SearchModal from "./components/SearchModal";
import SetupScreen from "./components/SetupScreen";
import Sidebar from "./components/Sidebar";
import { useTranslate, ensureLocale } from "./utils/i18n";
...
export default function App() {
  ...
  useEffect(() => {
    ...
    window.electron.getAppVersion().then((version) => {
      const lastVersion = localStorage.getItem("sinex_lastVersion");
      if (lastVersion && lastVersion !== version) {
        clearAppCaches();
      }
      localStorage.setItem("sinex_lastVersion", version);
    });
  }, []);
  ...
  // ── Load API key from secure storage on startup, fall back to default ──
  useEffect(() => {
    let mounted = true;
    secureStorage.get("apikey").then((val) => {
      ...
      if (val) {
        ...
        secureStorage.set("apikey", DEFAULT_API_KEY);
  ...
  useEffect(() => {
    ...
    (async function retryValidate() {
      for (let i = 0; i < 3; i++) {
        try {
          await tmdbFetch("/configuration", apiKey, { noLang: true, signal: controller.signal });
          setApiKeyStatus("ok");
          return;
        } catch (err) {
          if (err?.name === "AbortError" || controller.signal.aborted) return;
          if (i < 2) {
            console.warn(`TMDB validation attempt ${i + 1} failed, retrying...`);
            await new Promise(r => setTimeout(r, 1500));
          } else {
            pushError("TMDB validation", err?.message || err, err?.stack);
            console.error("TMDB validation fetch failed:", err);
  ...
  useEffect(() => {
    ...
    window.addEventListener("sinex:library-sort-changed", handler);
    return () =>
      window.removeEventListener("sinex:library-sort-changed", handler);
  }, []);
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("sinex:tmdb-lang-changed", handler);
  }, [fetchTrending]);
  useEffect(() => {
    // Theme
    const savedTheme = storage.get("theme") || "sinex";
  ...
  const navigate = useCallback((pg, data = null) => {
    ...
    setSelected(data);
    setPage(pg);
    if (pg !== "browse") setBrowseGenre(null);
    setShowSearch(false);
    setSidebarOpen(false);
  ...
  return (
    <ErrorBoundary>
      {showWelcome && <WelcomeScreen onDone={() => {
        ...
        setTimeout(() => setHeroReady(true), 5000);
      }} />}
      ...
      <div>
        ...
        <Sidebar
          ...
          onBack={navigateBack}
          onShowShortcuts={() => { setShowShortcuts(true); setSidebarOpen(false); }}
          mobile={isMobile}
        ...
        {isMobile && (
          <div className="mobile-nav">
            <button className={`mobile-nav-btn${page === "home" ? " active" : ""}`} onClick={() => { navigate("home"); }}>
              <HomeIcon />
            </button>
            <button className={`mobile-nav-btn search-btn${page !== "home" && page !== "history" ? "" : ""}`} onClick={() => { setShowSearch(true); }}>
              <SearchIcon />
            </button>
            <button className={`mobile-nav-btn${page === "history" ? " active" : ""}`} onClick={() => { navigate("history"); }}>
              <HistoryIcon />
            </button>
          </div>
        )}
        ...
        {updateBanner && (
          ...
          >
            <span style={{ display: "flex", alignItems: "center", gap: 6 }}><CelebrationIcon size={16} /> {t("app.updateBanner", { version: updateBanner.latest })}</span>
            {updateBanner.url && (
              <button
                onClick={() => window.electron?.openExternal(updateBanner.url)}
                style={{
                  color: "#fff",
                  fontWeight: 700,
              ...
              <div style={{ padding: "14px 18px" }}>
                ...
                >
                  ...
                  >
                    ...
                    {t("app.newEpisodesAvailable", { plural: episodeCheckStatus.entries.length > 1 ? "s" : "" })}
                  </div>
                  <button
                    onClick={() => {
                      clearTimeout(episodeDismissTimerRef.current);
              ...
              aria-label={t("app.dismiss")}
                ...
                >
                  {episodeCheckStatus.entries.slice(0, 5).map((entry) => (
                    ...
                    >
                      ...
                      {entry.season != null && (
                        ...
                        >
                          {t("tv.seasonLabel", { number: entry.season })}
                        </span>
                      )}
                    </li>
                  ))}
                  {episodeCheckStatus.entries.length > 5 && (
                    ...
                    >
                      {t("app.nMore", { n: episodeCheckStatus.entries.length - 5 })}
                    </li>
                  )}
                </ul>
              </div>
