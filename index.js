// -- SineX main process entry point ---------------------------------------
...
function createWindow() {
  ...
  const isDev = process.env.VITE_DEV_SERVER_URL || false;
  if (isDev) {
...
// -- YouTube video stream URL (yt-dlp, for hero background) ------------------
ipcMain.handle("get-video-url", async (_, videoId) => {
  ...
  try {
    ...
    const stdout = await new Promise((resolve, reject) => {
      execFile(
        ...
        [
          ...
          "-f",
          "bestvideo[ext=mp4][height<=1080]+bestaudio[ext=m4a]/best[height<=1080]",
        ],
    ...
    // Separate video + audio streams (1080p video + audio)
    if (data.requested_formats?.length === 2) {
      const v = data.requested_formats[0];
      const a = data.requested_formats[1];
      return { type: "separate", videoUrl: v.url, audioUrl: a.url };
    }

    // Single combined stream (usually 360p)
    return { type: "combined", videoUrl: data.url, audioUrl: null };
  } catch {
    return null;
  }
});
