...
# Frontend Skills (Loaded from Front-end Book)

## Anti-Slop Design Principles
- **DESIGN_VARIANCE:** 1-10 scale — controls layout experimentation. Higher = asymmetric/modern, lower = centered/clean.
- **MOTION_INTENSITY:** 1-10 scale — controls animation depth. Higher = scroll/magnetic, lower = hover-only.
- **VISUAL_DENSITY:** 1-10 scale — controls information per viewport. Higher = dense dashboards, lower = spacious.
- **Anti-Default Discipline:** Avoid AI-purple gradients, centered hero over dark mesh, three equal feature cards, generic glassmorphism, infinite micro-animations everywhere, Inter + slate-900. Read the brief first.
- **Layout Diversification:** Skip centered hero/H1 when VARIANCE > 4. Use split-screen, left-aligned content, or asymmetric whitespace.
- **Typography:** Discourage Inter for premium vibes. Use Geist, Outfit, Cabinet Grotesk, Satoshi. Serif = editorial only, banned for dashboards.
- **Color:** Max 1 accent color, saturation < 80%. No "AI Purple/Blue" aesthetic. Neutral bases (Zinc/Slate) with high-contrast singular accents.
- **The 3-Card Layout Ban:** Default 3-column card rows are banned. Use 2-column zig-zag, asymmetric grids, or horizontal scroll.
- **Viewport Stability:** Always use `min-h-[100dvh]` not `h-screen` for full-height sections (prevents iOS Safari layout jump).
- **Grid over Flex-Math:** Use CSS Grid (`grid grid-cols-1 md:grid-cols-3 gap-6`) not complex flexbox percentage math.
- **Interactive States:** Implement loading (skeletal), empty, error, and tactile feedback (`scale-[0.98]` on active).

## GSAP Animation Skills
- **Core:** `gsap.to()` / `from()` / `fromTo()` — animate transform aliases and `autoAlpha`. Register plugins once per app.
- **Timelines:** Use `gsap.timeline()` with position parameters (`+=0.2`, `-=0.1`). Prefer over chained delays.
- **ScrollTrigger:** Attach to timeline or top-level tween. Call `ScrollTrigger.refresh()` after DOM/layout changes.
- **React:** Use `useGSAP` hook from `@gsap/react` with `scope` for cleanup. Or `useEffect` + `gsap.context()` → `ctx.revert()`.
- **Performance:** Animate transforms/opacity only. Use `will-change` sparingly. Never animate `top`, `left`, `width`, `height`.
- **GSAP is 100% free** — all plugins (SplitText, MorphSVG, etc.) included in public `gsap` npm package.
- **Motion Philosophy:** Spring physics over linear easing. Use `type: "spring", stiffness: 100, damping: 20`. Stagger children for waterfall reveals.

## Liquid Glass Effect
- **Implementation:** Pure CSS SVG filters — `feDisplacementMap` + `backdrop-filter` for realistic refraction.
- **Chromatic Aberration:** RGB color splitting for premium glass look.
- **Props:** `depth` (refraction), `strength` (displacement), `blur`, `chromaticAberration` (RGB split).
- **Safari Fallback:** Auto degrades to glassmorphism when `backdrop-filter: url()` unsupported.
- **Anti-Slop Glass:** Beyond `backdrop-blur` — add 1px inner border (`border-white/10`) + inner shadow for edge refraction.

## DESIGN.md Pattern (Google Stitch Format)
- DESIGN.md is a plain-text design system document that AI agents read to generate consistent UI.
- Sections: Visual Theme & Atmosphere → Color Palette → Typography → Components → Layout → Depth → Do's/Don'ts → Responsive → Agent Prompt Guide.
- Copy a DESIGN.md into project root and tell the AI agent to use it for visually consistent output.

## Branding Awareness
- Branding > logo. Includes identity, colors, typography, illustration style, voice/tone, style guides.
- Open source projects need branding more than closed-source (no ad/marketing dept).
- Use design tokens: semantic name + hex + functional role.
- Maintain voice & tone documentation alongside visual components.
